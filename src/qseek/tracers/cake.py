...
class CakeTracer(RayTracer):
    ...
    async def prepare(
        ...
        receiver_depth_padding = np.ptp(receiver_depths) * 0.01  # 10% padding
        ...
        time_tolerance = octree.smallest_node_size() / (receiver_vmin.min() * 3)
        spatial_tolerance = octree.smallest_node_size() / 3
