...
from qseek.utils import PhaseDescription, datetime_now
...
class StationCorrections(BaseModel):
    ...
    def load_rundir(cls, rundir: Path) -> Self:
        corrections = cls(rundir=rundir)
        logger.debug("loading station detections from %s", rundir)
        detections = EventDetections.load_rundir(rundir)
        with console.status("aggregating station detections"):
            for event in detections:
                corrections.add_event(event)

        logger.info("aggregated %d station corrections", corrections.n_stations)
        if not corrections.n_stations:
            raise ValueError("no station detections found")

    @classmethod
    def from_file(cls, filename: Path) -> Self:
        logger.debug("loading station corrections from %s", filename)
        return cls.model_validate_json(filename.read_bytes())
    ...
    def n_stations(self) -> int:
        ...
        """
        return len(self.station_corrections)
    ...
    def save_plots(self, folder: Path) -> None:
        ...
        for correction in track(
            self.station_corrections.values(), description="plotting corrections"
        ):
            correction.plot(
